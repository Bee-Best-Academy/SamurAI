...
def train_gb_model():
    ...
    with open(f'GB_Auto_cyberbullying_model_{current_datetime}.json', 'w') as json_file:
        json.dump(model_dict, json_file)

    # Save the trained model
    joblib.dump(gb, f'GB_Auto_cyberbullying_model_{current_datetime}.pkl')
    ...
    with open('anonymization_cyberbullying_tweets_GB.csv', 'w', newline='', encoding='utf-8') as file:
...
def integrate_models(model_files):
    ...
    json_files = glob.glob('GB_Auto_cyberbullying_model_*.json')
    ...
    # Preprocess new data
    new_tweets_vectorized = gb_vectorizer.transform(new_tweets)

    # Aggregate predictions from all models
    all_predictions = []

    # Obtain predictions from the GB model
    gb_predictions = gb_model.predict(new_tweets_vectorized)
...
model_files = glob.glob('GB_Auto_cyberbullying_model_*.pkl')
