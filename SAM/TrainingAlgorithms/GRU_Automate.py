...
def train_gru_model():
    ...
    send_alert_created_model("GRU model has been created.")

    # Train a Gated Recurrent Unit classifier
    ...
    with open(f'GRU_Auto_cyberbullying_model_{current_datetime}.json', 'w') as json_file:
        json.dump(model_dict, json_file)

    # Save the trained model
    model.save(f'GRU_Auto_cyberbullying_model_{current_datetime}.h5')

    # Send alert for model save
    send_alert_saved_model("GRU Model has been saved.")

    # Evaluate the GRU model
    gru_predictions = model.predict(X_test)
    gru_accuracy = accuracy_score(y_test, np.argmax(gru_predictions, axis=1)) * 100
    gru_error_percentage = 100 - gru_accuracy

    # Display duration, accuracy, and error percentage
    print('GRU Classification Report:')
    print("Duration: {:.2f} minutes".format(duration_minutes))
    print("Accuracy: {:.2f}%".format(gru_accuracy))
    print("Error Percentage: {:.2f}%".format(gru_error_percentage))
    print()
    ...
    # Map numerical labels to the corresponding textual labels
    label_mapping = {
        0: 'age',
        1: 'ethnicity',
        2: 'gender',
    ...
    with open('anonymization_cyberbullying_tweets_GRU.csv', 'w', newline='', encoding='utf-8') as file:
        writer = csv.writer(file)
        writer.writerow(['tweet_text', 'cyberbullying_type'])

        for original_text, original_label in zip(original_texts, original_labels):
            writer.writerow([original_text, label_mapping[original_label]])

        for anonymized_text, anonymized_label in zip(anonymized_texts, anonymized_labels):
            writer.writerow([anonymized_text, label_mapping[anonymized_label]])
...
def integrate_models(model_files):
    # Load the GRU model
    gru_model = load_model(model_files[0])

    # Get a list of matching JSON file paths
    json_files = glob.glob('GRU_Auto_cyberbullying_model_*.json')
    ...
    if sorted_json_files:
        ...
        with open(most_recent_json_file, 'r') as json_file:
            gru_model_dict = json.load(json_file)
            gru_label_encoder = LabelEncoder()
            gru_label_encoder.classes_ = np.array(gru_model_dict['label_encoder'])
    else:
    ...
    # Preprocess new data
    new_tweets_preprocessed = [" ".join(tweet.lower().split()) for tweet in new_tweets]

    # Create a tokenizer and fit it on the preprocessed tweets
    tokenizer = Tokenizer()
    tokenizer.fit_on_texts(new_tweets_preprocessed)

    # Convert the preprocessed tweets to sequences of tokens
    sequences = tokenizer.texts_to_sequences(new_tweets_preprocessed)

    # Pad the sequences to have the same length
    max_sequence_length = 874
    padded_sequences = pad_sequences(sequences, maxlen=max_sequence_length)

    # Convert the padded sequences to tensors
    new_tweets_vectorized = tf.convert_to_tensor(padded_sequences)

    # Aggregate predictions from all models
    all_predictions = []

    # Obtain predictions from the GRU model
    gru_predictions = gru_model.predict(new_tweets_vectorized)
   ...
   # Get the sentiment labels from gru_label_encoder
    label_encoder_labels = gru_label_encoder.classes_
    ...
    final_predictions = gru_label_encoder.inverse_transform(final_predictions)
...
model_files = glob.glob('GRU_Auto_cyberbullying_model_*.h5')
