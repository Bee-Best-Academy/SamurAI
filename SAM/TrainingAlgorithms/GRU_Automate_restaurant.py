...
def train_gru_model():
    ...
    with open(f'GRU_Auto_restaurant_review_model_{current_datetime}.json', 'w') as json_file:
        json.dump(model_dict, json_file)

    # Save the trained model
    model.save(f'GRU_Auto_restaurant_review_model_{current_datetime}.h5')
    ...
    with open('anonymization_Restaurant_Reviews_GRU.csv', 'w', newline='', encoding='utf-8') as file:
...
def integrate_models(model_files):
    ...
    json_files = glob.glob('GRU_Auto_restaurant_review_model_*.json')
    ...
    # Preprocess new data
    new_reviews_preprocessed = [" ".join(tweet.lower().split()) for tweet in new_reviews]

    # Create a tokenizer and fit it on the preprocessed tweets
    tokenizer = Tokenizer()
    tokenizer.fit_on_texts(new_reviews_preprocessed)

    # Convert the preprocessed tweets to sequences of tokens
    sequences = tokenizer.texts_to_sequences(new_reviews_preprocessed)

    # Pad the sequences to have the same length
    max_sequence_length = 955
    padded_sequences = pad_sequences(sequences, maxlen=max_sequence_length)

    # Convert the padded sequences to tensors
    new_reviews_vectorized = tf.convert_to_tensor(padded_sequences)

    # Aggregate predictions from all models
    all_predictions = []

    # Obtain predictions from the GRU model
    gru_predictions = gru_model.predict(new_reviews_vectorized)
...
model_files = glob.glob('GRU_Auto_restaurant_review_model_*.h5')
