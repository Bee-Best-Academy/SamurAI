...
def train_lightgbm_model():
    ...
    # Separate the features (Review_Text) and labels (Rating)
    ...
    send_alert_created_model("LightGBM model has been created.")
    ...
    # Perform cross-validation without training the model
    random_search = RandomizedSearchCV(lgbm, param_distributions, n_iter=1, cv=2, random_state=42)
    cv_scores = cross_val_score(random_search, X_train_balanced, y_train_balanced, cv=3)
    ...
    lgbm = LGBMClassifier(random_state=42, **best_params)
    ...
    with open(f'LightGBM_Auto_restaurant_review_model_{current_datetime}.json', 'w') as json_file:
        json.dump(model_dict, json_file)

    # Save the trained model
    joblib.dump(lgbm, f'LightGBM_Auto_restaurant_review_model_{current_datetime}.pkl')

    # Send alert for model save
    send_alert_saved_model("Light GBM Model has been saved.")
    ...
    print("Error Percentage: {:.2f}%".format(100 - cv_scores.mean() * 100))
    print()
    ...
    with open('anonymization_restaurant_review_LightGBM.csv', 'w', newline='', encoding='utf-8') as file:
...
def integrate_models(model_files):
    ...
    lgbm_model = joblib.load(model_files[0])

    # Get a list of matching JSON file paths
    json_files = glob.glob('LightGBM_Auto_restaurant_review_model_*.json')
    ...
    if sorted_json_files:
        ...
        with open(most_recent_json_file, 'r') as json_file:
            lgbm_model_dict = json.load(json_file)
            lgbm_vectorizer_serialized = lgbm_model_dict['vectorizer']
            lgbm_vectorizer = pickle.loads(base64.b64decode(lgbm_vectorizer_serialized))
            lgbm_label_encoder = LabelEncoder()
            lgbm_label_encoder.classes_ = np.array(lgbm_model_dict['label_encoder'])
    else:
    ...
    # Preprocess new data
    new_reviews_vectorized = lgbm_vectorizer.transform(new_reviews)

    # Aggregate predictions from all models
    all_predictions = []

    # Obtain predictions from the LightGBM model
    lgbm_predictions = lgbm_model.predict(new_reviews_vectorized)
    all_predictions.append(lgbm_predictions)
   ...
   # Get the sentiment labels from lgbm_label_encoder
    label_encoder_labels = lgbm_label_encoder.classes_
    ...
    final_predictions = lgbm_label_encoder.inverse_transform(final_predictions)
...
model_files = glob.glob('LightGBM_Auto_restaurant_review_model_*.pkl')
...
# Call integrate_models function with thresholds
