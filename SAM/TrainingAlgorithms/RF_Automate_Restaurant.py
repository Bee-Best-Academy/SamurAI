...
def train_rf_model():
    ...
    with open(f'RF_Auto_restaurant_review_model_{current_datetime}.json', 'w') as json_file:
        json.dump(model_dict, json_file)

    # Save the trained model
    joblib.dump(rf, f'RF_Auto_restaurant_review_model_{current_datetime}.pkl')
    ...
    with open('anonymization_Restaurant_Reviews_RF.csv', 'w', newline='', encoding='utf-8') as file:
...
def integrate_models(model_files):
    ...
    json_files = glob.glob('RF_Auto_restaurant_review_model_*.json')
    ...
    # Preprocess new data
    new_reviews_vectorized = rf_vectorizer.transform(new_reviews)

    # Aggregate predictions from all models
    all_predictions = []

    # Obtain predictions from the RF model
    rf_predictions = rf_model.predict(new_reviews_vectorized)
...
model_files = glob.glob('RF_Auto_restaurant_review_model_*.pkl')
