...
def train_svm_model():
    ...
    send_alert_created_model("SVM model has been created.")
    ...
    with open(f'SVM_Auto_cyberbullying_model_{current_datetime}.json', 'w') as json_file:
        json.dump(model_dict, json_file)

    # Save the trained model
    joblib.dump(svm, f'SVM_Auto_cyberbullying_model_{current_datetime}.pkl')

    # Send alert for model save
    send_alert_saved_model("SVM Model has been saved.")

    # Evaluate the SVM model
    svm_predictions = svm.predict(X_test)
    svm_accuracy = accuracy_score(y_test, svm_predictions) * 100
    svm_error_percentage = 100 - svm_accuracy
    ...
    print("Error Percentage: {:.2f}%".format(svm_error_percentage))
    print()
    ...
    with open('anonymization_cyberbullying_tweets_SVM.csv', 'w', newline='', encoding='utf-8') as file:
...
def integrate_models(model_files):
    # Load the SVM model
    svm_model = joblib.load(model_files[0])

    # Get a list of matching JSON file paths
    json_files = glob.glob('SVM_Auto_cyberbullying_model_*.json')
    ...
    if sorted_json_files:
        ...
        with open(most_recent_json_file, 'r') as json_file:
            svm_model_dict = json.load(json_file)
            svm_vectorizer_serialized = svm_model_dict['vectorizer']
            svm_vectorizer = pickle.loads(base64.b64decode(svm_vectorizer_serialized))
            svm_label_encoder = LabelEncoder()
            svm_label_encoder.classes_ = np.array(svm_model_dict['label_encoder'])
    else:
    ...
    # Preprocess new data
    new_tweets_vectorized = svm_vectorizer.transform(new_tweets)

    # Aggregate predictions from all models
    all_predictions = []

    # Obtain predictions from the SVM model
    svm_predictions = svm_model.predict(new_tweets_vectorized)
   ...
   # Get the sentiment labels from svm_label_encoder
    label_encoder_labels = svm_label_encoder.classes_
    ...
    final_predictions = svm_label_encoder.inverse_transform(final_predictions)
...
model_files = glob.glob('SVM_Auto_cyberbullying_model_*.pkl')
