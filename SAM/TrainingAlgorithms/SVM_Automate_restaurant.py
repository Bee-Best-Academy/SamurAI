...
def train_svm_model():
    ...
    with open(f'SVM_Auto_restaurant_review_model_{current_datetime}.json', 'w') as json_file:
        json.dump(model_dict, json_file)

    # Save the trained model
    joblib.dump(svm, f'SVM_Auto_restaurant_review_model_{current_datetime}.pkl')
    ...
    with open('anonymization_Restaurant_Reviews_SVM.csv', 'w', newline='', encoding='utf-8') as file:
...
def integrate_models(model_files):
    ...
    json_files = glob.glob('SVM_Auto_restaurant_review_model_*.json')
    ...
    # Preprocess new data
    new_reviews_vectorized = svm_vectorizer.transform(new_reviews)
...
model_files = glob.glob('SVM_Auto_restaurant_review_model_*.pkl')
