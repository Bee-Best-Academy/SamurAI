...
def train_USE_model():
    ...
    # Encode the review text using the Universal Sentence Encoder
    review_embeddings = embed(reviews).numpy()

    # Encode the labels
    label_encoder = LabelEncoder()
    y_encoded = label_encoder.fit_transform(rating)

    # Split the data into training and testing sets
    X_train, X_test, y_train, y_test = train_test_split(review_embeddings, y_encoded, test_size=0.2, random_state=42)
    ...
    with open(f'USE_Auto_restaurant_review_model_{current_datetime}.json', 'w') as json_file:
        json.dump(model_dict, json_file)

    # Save the trained model
    joblib.dump(classifier, f'USE_Auto_restaurant_review_model_{current_datetime}.pkl')
    ...
    X_test_anonymized = [anonymize_text(text) for text in reviews]
    ...
    for original_text, anonymized_text, label in zip(reviews, X_test_anonymized, rating):
    ...
    with open('anonymization_restaurant_review_USE.csv', 'w', newline='', encoding='utf-8') as file:
...
def integrate_models(model_files):
    ...
    json_files = glob.glob('USE_Auto_restaurant_review_model_*.json')
    ...
    new_reviews_vectorized = embed(new_reviews).numpy()

    # Aggregate predictions from all models
    all_predictions = []

    # Obtain predictions from the SVM model
    svm_predictions = use_model.predict(new_reviews_vectorized)
...
def continuous_training():
    while True:
        ...
        if is_new_data_added(existing_df):
            ...
            current_datetime = pd.Timestamp.now().strftime("%Y%m%d_%H%M%S")
            new_dataset_file = f'Restaurant_Reviews{current_datetime}.csv'
...
model_files = glob.glob('USE_Auto_restaurant_review_model_*.pkl')
